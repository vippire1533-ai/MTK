...
namespace WebTraiCay.Areas.Admin.Controllers
{
    [CustomAuthorizeAttribute(RoleID = "SALESMAN")]
    public class ProductsController : BaseController
    {
        private WebTraiCayDbContext db = new WebTraiCayDbContext();
        ...
        public ActionResult Edit( Mproduct mproduct, HttpPostedFileBase file)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                link tt_link = db.Link.Where(m => m.slug == mproduct.slug).FirstOrDefault();
                if(tt_link == null)
                {
                    link tt_link1 = new link();
                    tt_link1.slug = slug;
                    tt_link1.tableId = 1;
                    tt_link1.type = "ProductDetail";
                    tt_link1.parentId = mproduct.ID;
                    db.Link.Add(tt_link1);
                }
                else
                {
                    var thisLink = db.Link.Where(m => m.tableId == 1 && m.parentId == mproduct.ID).First();
                    link tt_link3 = db.Link.Find(thisLink.ID);
                    tt_link3.slug = slug;
                    tt_link3.tableId = 1;
                    tt_link3.type = "ProductDetail";
                    tt_link3.parentId = mproduct.ID;
