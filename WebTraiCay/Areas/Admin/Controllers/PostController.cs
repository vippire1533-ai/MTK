    ...
    public class PostController : BaseController
    {
        ...
        public ActionResult Edit( Mpost mpost)
        {
            if (ModelState.IsValid)
            {
                ...
                link tt_link = db.Link.Where(m => m.slug == mpost.slug).FirstOrDefault();
                if (tt_link == null)
                {
                    link tt_link1 = new link();
                    tt_link1.slug = slug;
                    tt_link1.tableId = 4;
                    tt_link1.type = "PostDetail";
                    tt_link1.parentId = mpost.ID;
                    db.Link.Add(tt_link1);
                }
                else
                {
                    var thisLink = db.Link.Where(m => m.tableId == 4 && m.parentId == mpost.ID).First();
                    link tt_link3 = db.Link.Find(thisLink.ID);
                    tt_link3.slug = slug;
                    tt_link3.tableId = 1;
                    tt_link3.type = "PostDetail";
                    tt_link3.parentId = mpost.ID;
